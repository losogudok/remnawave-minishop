...
- отдельные переключатели и максимумы для слайдеров устройств, обычного и premium-трафика на этапе оформления подписки.
...
Слайдеры строятся из дискретных вариантов тарифа. Минимальная отметка равна ресурсу, включённому в тариф, а максимум должен точно совпадать с одной из настроенных отметок.

- устройства показываются только для конечного положительного HWID-лимита, включённого раздела устройств и валидных параметров `checkout_addons.devices`; шаг всегда равен одному устройству, а существующие `hwid_device_packages` используются только отдельным сценарием докупки после оформления;
- обычный трафик показывается только при положительном ограниченном `monthly_gb` и `flexible_traffic_limit`;
- premium-трафик показывается только при premium squads, выключенном `premium_unlimited` и `premium_flexible_traffic_limit`;
- для Stars остаются только отметки, у которых настроена цена Stars;
- провайдеры с внешней ценой или управляемой ими рекуррентной подпиской скрывают докупки, если не умеют включить их в первый платёж.

`flexible_traffic_limit` и `premium_flexible_traffic_limit` задают **итоговый лимит на каждый период сброса**, а не одноразовый объём. Отметки слайдера генерируются равномерно: базовый лимит, затем `step_gb` до `max_total_gb`. Ежемесячная доплата равна числу выбранных шагов × `price_per_step` (или `stars_price_per_step`). Например, база `100 GB`, шаг `50 GB` и максимум `300 GB` дают варианты `100/150/200/250/300 GB`. Устройства строятся аналогично, но с фиксированным шагом `1`: ежемесячная доплата равна выбранному числу устройств × `price_per_device` (или `stars_price_per_device`). Отдельные `topup_packages`, `premium_topup_packages` и `hwid_device_packages` остаются существующими сценариями докупки после оформления и не используются слайдерами checkout.

Цена гибкого варианта считается как ежемесячная доплата × число покупаемых месяцев. При раннем продлении того же тарифа будущий срок оплачивается полностью, а положительное увеличение лимита или числа устройств на уже оплаченном остатке срока добавляется пропорционально оставшимся дням. Понижение не создаёт возврат и вступает в силу с начала нового оплаченного окна. Уже купленные более высокие права не затираются более низкими пересекающимися окнами. При нескольких ранних продлениях подряд доплата за трафик считается отдельно для каждого уже оплаченного временного окна; повторный выбор устройств скрывается, пока не начнётся ранее запланированное HWID-окно, чтобы не сложить перекрывающиеся пакеты дважды.

Backend пересчитывает корзину на каждом изменении и повторно проверяет её при создании платежа. В `Payment.checkout_bundle_snapshot` версии 2 сохраняются тариф, граница активной подписки, период, базовая цена и выбранные итоговые лимиты; `checkout_bundle_hash` участвует в безопасном переиспользовании pending-платежа. Если подписка изменилась до активации, устаревший снимок отклоняется. Изменение каталога после создания счёта не меняет уже оплаченную выдачу. Прямое продление с другим `tariff_key` отклоняется: смена тарифа проходит через отдельный сценарий конвертации/доплаты.

Активные гибкие лимиты хранятся отдельными временными окнами в `flexible_traffic_limits`. Автопродление переносит действующий вариант и его зафиксированную ежемесячную цену в следующий срок. Отдельный top-up баланс при этом не повторяется и не обнуляется.
