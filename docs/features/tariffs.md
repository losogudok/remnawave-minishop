...
Приветственный бонус приглашённому (`REFERRAL_WELCOME_BONUS_DAYS`) настраивается в отдельном блоке **Реферальная программа** на странице тарифов. Поле каталога `referral_welcome_bonus_tariff` выбирает включённый period-тариф для новой бонусной подписки; если оно отсутствует, используется `default_tariff`. Из тарифа наследуются обычный и premium-лимиты трафика, Internal Squads, стратегия сброса и HWID-лимит. Там же флаги `REFERRAL_WEBAPP_LINK_ENABLED` и `REFERRAL_TELEGRAM_LINK_ENABLED` определяют, какие реферальные ссылки видит пользователь в разделе бонусов Web App; хотя бы один из них всегда должен оставаться включённым. `REFERRAL_WELCOME_BONUS_WITHOUT_TELEGRAM_ENABLED` разрешает или запрещает выдачу приветственного бонуса аккаунтам без Telegram; disposable email домены из `DISPOSABLE_EMAIL_DOMAINS` всегда требуют Telegram перед начислением.
