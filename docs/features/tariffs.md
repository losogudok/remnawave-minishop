...
- снятие сквадов останавливает только новые подключения, поэтому сразу после ограничения бот просит панель разорвать уже установленные сессии этого пользователя на premium-нодах (`POST /api/ip-control/drop-connections`, только по premium-нодам — обычный доступ и чужие сессии не затрагиваются). Отключается настройкой `TARIFF_PREMIUM_DROP_CONNECTIONS`, повтор ограничен `TARIFF_PREMIUM_DROP_CONNECTIONS_COOLDOWN_SECONDS`. **Для этого ноды Remnawave должны быть запущены с `CAP_NET_ADMIN`** — см. раздел ниже;
