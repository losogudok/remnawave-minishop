...
# Reuse TELEGRAM_BOT_PROXY_URL for server-side token/JWKS requests by default.
...
Если сервер не может напрямую открыть `oauth.telegram.org`, задайте SOCKS5 endpoint. Server-side
часть OAuth использует его автоматически:
...
Чтобы Bot API продолжал использовать proxy, а OAuth ходил напрямую, задайте
...
- OAuth работает до callback, но завершается `invalid_token`: проверьте доступ `backend` к token
  endpoint и JWKS; при настроенном proxy проверьте `TELEGRAM_BOT_PROXY_URL` и убедитесь, что
  `TELEGRAM_OAUTH_USE_BOT_PROXY` не выключен.
