...
| `PARTNER_CLIENT_PAYMENT_BONUS_ENABLED` | Начисляет клиенту партнёра дни из колонки бонуса приглашённому в матрице его тарифа. Наследует `REFERRAL_ONE_BONUS_PER_REFEREE`; по умолчанию `False`. |
