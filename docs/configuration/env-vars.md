...
| `PARTNER_CLIENT_PAYMENT_BONUS_ENABLED` | Начисляет клиенту партнёра дни из колонки бонуса приглашённому в матрице его тарифа; по умолчанию `False`. |
| `PARTNER_ONE_BONUS_PER_CLIENT` | Если включено, бонусные дни по тарифной матрице начисляются только за первый успешный платёж клиента партнёра. Не влияет на обычную реферальную программу; по умолчанию `True`. |
