...
| `PARTNER_BALANCE_PAYMENT_ENABLED` | Разрешает полностью оплатить продление действующей period-подписки партнёрским балансом. Частичная оплата не поддерживается; по умолчанию `True`. |
